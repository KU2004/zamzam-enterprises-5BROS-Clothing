...
const productVariants = [
  {
    ...
    technicalHighlights: [
      ...
      { title: "Customization", value: "Custom labels, tags, and full branding options" },
      { title: "MOQ", value: "Low MOQ for private label launches" },
    ],
    benefits: [
  ...
  },
  {
    ...
    subtitle: "High-quality blanks for brands seeking exceptional softness and durability in their collections.",
    image: custom3Bg,
    imageP2: custom3p2,
    bgImage: custom3Bg,
    technicalHighlights: [
  ...
  return (
    <>
      <Seo
        ...
        description={`${product.subtitle} Explore the product details, customization options, and request a quote for this custom label option.`}
        canonicalPath={`/products/custom-label/details/${product.id}`}
        keywords={`${product.title.toLowerCase()}, private label manufacturing, OEM apparel, custom label specifications`}
      />
            ...
            <div className="self-start xl:-ml-2">
              <div className="sticky top-20 self-start">
                <div className="h-fit self-start rounded-[1.7rem] border border-border/60 bg-card p-4 shadow-[0_12px_30px_-18px_rgba(15,23,42,0.16)] sm:p-5">
                  <p className="mb-4 text-sm text-gold">Custom label manufacturing</p>
          ...
          <div className="rounded-[2.1rem] border border-border bg-card p-8 shadow-[0_15px_50px_-20px_rgba(0,0,0,0.08)]">
            <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <h2 className="mt-2 font-display text-3xl">Explore more custom label options</h2>
              </div>
              <Link to="/products/custom-label" className="inline-flex items-center justify-center rounded-[0.95rem] bg-charcoal px-6 py-3 text-sm font-semibold text-white transition hover:bg-charcoal/90">
            ...
            <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
              {relatedProducts.map((item) => (
                <Link key={item.id} to={`/products/custom-label/details/${item.id}`} className="group overflow-hidden rounded-[1.25rem] border border-border bg-background transition hover:-translate-y-1 hover:shadow-lg">
